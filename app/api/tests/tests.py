...
def _calc_revenue(click, conversion) -> int:
    rev = 0
    data = _get_configuration_body()
    rev += data['impression_revenue']
    if click:
        rev += data['click_revenue']
    if conversion:
        rev += data['conversion_revenue']
    return rev
...
def _send_win_and_check_response(request_id, bid_response_body):
    ...
    if data['win']:
        data["revenue"] = _calc_revenue(data['click'], data['conversion'])
    response = requests.post(RTB_NOTIFY_URL, json=data)
    assert response.status_code == 200
...
def test_script_game():
    body = _get_configuration_body(impressions_total=20, mode="script")
    response = requests.post(GAME_CONFIGURATION_URL, json=body)
    assert response.status_code == 200
    ...
    bid_request_body = {
        "id": "",
        "imp": {"banner": {"w": 250, "h": 250}},
        "bcat": [],
        "site": {"domain": "www.example.com"}, "ssp": {"id": "0938831"}, "user": {"id": "u_cq_001_87311"},
    }
    for i in range(1, body['impressions_total']):
        bid_request_body["id"] = "id_{}".format(str(i))
        bid_request_body['click'] = {"prob":  round(random.uniform(0.01, 0.99), 2)}
        bid_request_body['conv'] = {"prob":  round(random.uniform(0.01, 0.99),2)}
        ...
        if bid_response_body:
            ...
            _send_win_and_check_response("id_{}".format(str(i)), bid_response_body)

def test_free_game():
    body = _get_configuration_body(impressions_total=20, mode="free")
    response = requests.post(GAME_CONFIGURATION_URL, json=body)
    assert response.status_code == 200
    ...
    for i in range(1, body['impressions_total']):
        id = "id_{}".format(str(i))
        bid_request_body["id"] = id
        bid_request_body['click'] = {"prob":  round(random.uniform(0.01, 0.99), 2)}
        bid_request_body['conv'] = {"prob":  round(random.uniform(0.01, 0.99),2)}
        bid_response_body = _send_bid_request_and_check_response(bid_request_body)
        if bid_response_body:
            _send_win_and_check_response(id, bid_response_body)
